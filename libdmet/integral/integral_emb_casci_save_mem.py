...
def __transform01(v_A, v_B, u_A, u_B, h_0, h_A, h_B, D):
    H0_H0 = h_0
...
def __transform2(v_A, v_B, u_A, u_B, w):
    ...
    H0_H2 = 1.0*np.sum(val008*val007) + \
            1.0*np.sum(val002*val004) + \
            0.5*np.sum(val005*val004) + \
            -0.5*np.sum(val003*val001) + \
    ...
    H1D_H2 = 1.0*np.dot(val013, v_B) + \
            1.0*np.dot(val012, v_B) + \
            -1.0*np.dot(val015, v_B) + \
            -1.0*np.dot(val010, u_A) + \
    ...
    val023 = -0.5*val019 + \
            ...
            0.5*np.transpose(val019, (1, 0, 2, 3)) + \
    ...
    val028 = 0.5*np.tensordot(np.tensordot(val027, u_B, axes=(3,0)), u_B, axes=(2,0)) + \
            -1.0*np.tensordot(np.tensordot(val025, u_B, axes=(3,0)), u_B, axes=(2,0)) + \
    ...
    val030 = -1.0*np.tensordot(np.tensordot(val029, u_A, axes=(3,0)), u_A, axes=(2,0)) + \
            0.5*np.tensordot(np.tensordot(val029, v_B, axes=(3,0)), v_B, axes=(2,0)) + \
            0.5*np.tensordot(np.tensordot(np.tensordot(u_A, val018, axes=(0,1)), u_A, axes=(3,0)), u_A, axes=(2,0))
    ...
    val037 = 0.5*np.transpose(val034, (1, 0, 2, 3)) + \
            -0.5*val034 + \
    ...
    val042 = 0.5*np.transpose(val038, (0, 1, 3, 2)) + \
            0.5*np.transpose(val039, (0, 1, 3, 2)) + \
            -1.0*val041
    val043 = val042 - np.transpose(val042, (1,0,2,3))
    H2x_H2 = val043 - np.transpose(val043, (0,1,3,2))
    H2wAB_H2 = 1.0*np.tensordot(np.tensordot(val027, u_A, axes=(3,0)), u_A, axes=(2,0)) + \
            -1.0*val040 + \
...
def transform(v_A, v_B, u_A, u_B, h_0, h_A, h_B, D, w):
    H0_1, H1_1, D_1 = __transform01(v_A, v_B, u_A, u_B, h_0, h_A, h_B, D)
    nImp = w.shape[0]
    H0_2, H1_2, D_2, w_2, y_2, x_2 = __transform2(v_A[:nImp], v_B[:nImp], \
            u_A[:nImp], u_B[:nImp], w)
    return H0_1+H0_2, H1_1+H1_2, D_1+D_2, w_2, y_2, x_2
