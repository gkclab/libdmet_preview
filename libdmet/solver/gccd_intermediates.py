...
def cc_Foo(t1, t2, eris):
    ...
    tau_tilde = make_tau(t2, t1, t1, fac=0.5)
    Fmi  = 0.5 * einsum('inef,mnef->mi', tau_tilde, eris.oovv)
    Fmi += foo
    return Fmi
...
def Wooov(t1, t2, eris):
    Wmnie = np.array(eris.ooov, copy=True)
    return Wmnie

def Wvovv(t1, t2, eris):
    Wamef = -np.array(eris.ovvv, copy=True).transpose(1,0,2,3)
    return Wamef
