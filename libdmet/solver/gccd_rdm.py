...
def _gamma2_intermediates(mycc, t1, t2, l1, l2):
    ...
    #goovv += einsum('cb,ijca->ijab', tmp * 0.5, t2)
    #tmp = None

    #tmp = einsum('kc,jc->kj', l1, t1)
    #goovv += einsum('kiab,kj->ijab', tau, tmp * 0.5)
    #tmp = None

    tmp = np.einsum('ldjd->lj', miajb)
    goovv -= einsum('lj,liba->ijab', tmp * 0.25, tau)
    tmp = None

    tmp = np.einsum('ldlb->db', miajb)
    goovv -= einsum('db,jida->ijab', tmp * 0.25, tau)
    tmp = None
    ...
    #tmp = None
    #gooov += einsum('icja,kc->jkia', miajb, t1 * 0.5)
    #gooov = gooov.conj()
    #gooov += einsum('jkab,ib->jkia', l2, t1 * 0.25)
    ...
    #govvv  = einsum('ja,ijcb->iacb', l1 * 0.25, tau)
    #govvv += einsum('bcad,id->iabc', gvvvv, t1)
    #tmp = np.einsum('kakb->ab', miajb) * .25
    #govvv += einsum('ab,ic->iacb', tmp, t1)
    #govvv += einsum('kaib,kc->iabc', miajb, t1 * 0.5)
    #govvv = govvv.conj()
    #govvv += einsum('ijbc,ja->iabc', l2, t1 * 0.25)
