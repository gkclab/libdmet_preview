...
'''
Test DMET with CSC from global density matrix.
'''
...
def test_csc():
    """
    Test charge self-consistency with H6 321G, FCI@RHF.
    """
    ...
    for iter in range(MaxIter):
        ...
        # CSC based on global density matrix
        veff, veff_ao, rdm1_glob = slater.get_veff_from_rdm1_emb(Lat, rhoEmb,
                basis, return_update=True)
        Lat.update_Ham(rdm1_glob, veff=veff_ao, vhf=veff_ao)
    ...
    assert abs(EnergyImp - -1.242180528205) < 1e-4
