...
def test_cc_solver(incore, ccd):
    ...
    for iter in range(MaxIter):
        ...
        ImpHam, H1e, basis = dmet.ConstructImpHam(Lat, rho, vcor, matching=True, int_bath=int_bath, \
                add_vcor=False, orth=True, incore=incore)
        ...
        solver_args = {"nelec": (Lat.ncore+Lat.nval)*2, \
                ...
                "ccsdt": False, "ccsdt_energy": False, "ccd": ccd}
        ...
        vcor_new, err = dmet.FitVcor(rhoEmb, Lat, basis, \
                ...
                imp_fit=imp_fit, ytol=ytol, gtol=gtol, CG_check=CG_check, test_grad=True)
    ...
    if ccd:
        assert abs(EnergyImp*nscsites/float(ncell_sc) - -1.242043057334) < 1e-4
    else:
...
if __name__ == "__main__":
    test_cc_solver(incore=True, ccd=False)
