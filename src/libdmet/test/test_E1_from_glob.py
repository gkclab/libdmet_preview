...
def test_E1_from_glob():
    ...
    FCI = dmet.impurity_solver.CCSD(restricted=restricted, tol=1e-8)
    solver = FCI
    nelec_tol = 5.0e-6
    delta = 0.01
    step = 0.1
    ...
    for iter in range(MaxIter):
        ...
        #rdm1_emb = slater.foldRho_k(Lat.R2k(rdm1_glob_R), basis_k) * 0.5
        ...
        E1 = E1.real
        #E1 = np.einsum('skpq, skqp ->', Lat.getH1(kspace=True), rdm1_glob_k) / nkpts
        #
        #hcore_R = Lat.getH1(kspace=False)
        #E = 0.0
        #spin = 1
        #for s in range(spin):
        ...
        rhoImp, EnergyImp, nelecImp = \
                ...
                vcor=vcor, E1=E1, rebuild_veff=True)
        EnergyImp *= nscsites
        ...
        if dVcor_per_ele < u_tol and abs(dE) < E_tol and iter > iter_tol :
            conv = True
            break
    assert abs(EnergyImp - -1.242066325237) < 5e-5
